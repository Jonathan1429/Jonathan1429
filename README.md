<h1 align="center">Hola, soy Jonathan EV <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35"></h1>
<!--<h1 align="center">José Jonathan Escobar Vázquez</h1>-->
<p align="center">
    <img src="https://readme-typing-svg.herokuapp.com?font=Time+New+Roman&color=%23C8BE25&size=25&center=true&vCenter=true&width=600&height=100&lines=Android+Developer+especializado+en+Kotlin;Arquitectura+limpia+y+código+mantenible;Fuerte+enfoque+en+Testing+Unitario;Aplicaciones+estructuradas+y+escalables;Ingeniero+en+Sistemas+Computacionales;Resolviendo+retos+técnicos+con+claridad">
</p>

<div>
  <!--<img align="right" 
       src="https://github.com/7oSkaaa/7oSkaaa/blob/main/Images/Right_Side.gif?raw=true" 
       width="250px" 
       style="margin-top: 10px;" />-->

<p align="center"><img src="https://raw.githubusercontent.com/KevinPatel04/KevinPatel04/master/cover-thompson.png"></p>       

  <h2>👨‍💻 Sobre mí</h2>

  <p>
    Desarrollador Android especializado en Kotlin, enfocado en la construcción
    de aplicaciones estructuradas y mantenibles.
  </p>

  <p>
    Aplico principios de Clean Architecture y el patrón MVVM para lograr
    separación clara de responsabilidades y facilitar la evolución del software.
  </p>

  <p>
    Tengo un fuerte interés en la calidad del código y el testing unitario,
    implementando pruebas en la lógica de negocio para asegurar confiabilidad
    y reducir errores.
  </p>

  <p>
    Me motiva comprender el funcionamiento interno de las soluciones que desarrollo
    y mejorar continuamente mis prácticas de arquitectura y calidad de software.
  </p>
</div>

---

## 🛠 Stack Tecnológico

### 💻 Lenguaje
![Kotlin](https://img.shields.io/badge/-Kotlin-0D1117?style=flat&logo=kotlin)&nbsp;

### 📱 Desarrollo Android
- XML
- MVVM
- Navigation Component
- Coroutines
- Flow

### 🏗 Arquitectura
- Clean Architecture aplicada
- Use Cases
- Repository Pattern
- Separación por capas (Presentación, Dominio, Datos)
- Inyección de dependencias

### 🧪 Testing
- Testing Unitario
- JUnit
- MockK
- Pruebas de UseCases
- Aislamiento de dependencias mediante mocks

### 🛠 Herramientas
![Git](https://img.shields.io/badge/-Git-000?&logo=Git)
![GitHub](https://img.shields.io/badge/-GitHub-000?&logo=GitHub)
![Android Studio](https://img.shields.io/badge/-Android%20Studio-05122A?style=flat&logo=android-studio&logoColor=3DDC84)&nbsp;

---

## 🚀 Proyecto Destacado

### Review

Aplicación Android desarrollada en Kotlin, estructurada bajo el patrón MVVM y principios de Clean Architecture.

**Características técnicas:**

- Arquitectura multicapa bien definida
- Separación clara entre presentación, dominio y datos
- Lógica de negocio desacoplada
- Implementación de testing unitario en casos de uso
- Publicada en Google Play Store

[![Github](https://img.shields.io/badge/-Review-black?style=flat&labelColor=black&logo=github&logoColor=white)](https://github.com/Jonathan1429/Review)
[![Google Playstore](https://img.shields.io/badge/-Review-gray?style=flat&logo=Google-Play&logoColor=white)](https://play.google.com/store/apps/details?id=com.jonathanev.review&hl=es_MX)

---

## 🧠 Enfoque Profesional

- Código limpio y estructurado
- Enfoque en mantenibilidad y escalabilidad
- Testing como parte fundamental del desarrollo
- Interés constante por mejorar arquitectura y calidad de software
- Resolución de retos técnicos y optimización de rendimiento

---

<h1>Reconocimientos! 🏆</h1>
<Br>

[![Certificado_Programacion Badge](https://img.shields.io/badge/-Certificado%20de%20Programacion-blue?style=flat&logo=Linkedin&logoColor=white&link=https://github.com/Jonathan1429/Jonathan1429/blob/main/CCUPTECNM2020-1236-JOS%C3%89%20JONATHAN%20ESCOBAR%20V%C3%81ZQUEZ.pdf)](https://github.com/Jonathan1429/Jonathan1429/blob/main/CCUPTECNM2020-1236-JOS%C3%89%20JONATHAN%20ESCOBAR%20V%C3%81ZQUEZ.pdf)
## 📌 Filosofía

> "El esfuerzo vence al talento cuando el talento no se está esforzando."

---

## 📫 Contacto
[![Linkedin Badge](https://img.shields.io/badge/LinkedIn-JonathanEV-blue?style=flat&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/jonathan-escobar-5315a0206/)](https://www.linkedin.com/in/jonathan-escobar-5315a0206/)
[![Gmail Badge](https://img.shields.io/badge/-JonathanEV-c14438?style=flat&logo=Gmail&logoColor=white&link=mailto:jona142959@gmail.com)](mailto:jona142959@gmail.com)
